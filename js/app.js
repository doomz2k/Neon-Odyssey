...
// Player-side logic. Talks directly to Supabase via the `sb` client created in
// supabase-config.js. No login yet (Eapheron pattern) — a player just sets a
// display name, remembered on this device, which tags the characters they make.
...
function playerName() { try { return localStorage.getItem(PLAYER_KEY) || ''; } catch (e) { return ''; } }
function setPlayerName(v) { try { localStorage.setItem(PLAYER_KEY, v); } catch (e) {} }
...
function initPlayer() {
  ...
  input.onchange = () => setPlayerName(input.value.trim());
}
...
async function loadCharacters() {
  ...
  const { data, error } = await sb
    .from('characters')
    .select('id,name,species,class_name,subclass,level,player_name')
    .order('created_at', { ascending: false });
  if (error) { el.innerHTML = `<div class="msg error">${esc(error.message)}</div>`; return; }
  el.innerHTML = data.length ? '' : '<p class="muted">No characters yet — create the first Outrunner above.</p>';
  for (const c of data) {
    ...
    div.innerHTML = `<h3>${esc(c.name)}</h3>
      ...
      <div class="meta">Player: ${esc(c.player_name || '—')}</div>
...
async function loadNotes() {
  const el = $('noteList');
  // Player pages only ever request shared notes.
  const { data, error } = await sb
...
async function postNote() {
  ...
  const { error } = await sb.from('notes').insert({
    author_label: playerName() || 'A player',
    title, body: $('n-body').value, visibility: 'shared',
  });
